...
$data2=$sql->query('SELECT count(*) AS nb_comm FROM hg3_comment', TRUE);
...
$req=$sql->query('SELECT hg3_comment.id AS id_comm, hg3_comment.id_img, hg3_comment.date, hg3_comment.name AS auteur, hg3_comment.comment, hg3_img.id AS id_img, hg3_img.name FROM hg3_comment LEFT JOIN hg3_img ON hg3_img.id=hg3_comment.id_img'.$where.' ORDER BY hg3_comment.date DESC LIMIT '.$l0.', '.NB_COMM_PER_PAGE);
