...
if(isset($_POST['list']) && !empty($_POST['list'])){
  foreach($_POST['list'] as $k=>$v){
    $data2=$sql->query('SELECT hg3_img.id AS id_img, hg3_img.file, hg3_img.name AS name_img, hg3_cat.id AS id_cat, hg3_cat.name AS name_cat, hg3_cat.link
      FROM hg3_img LEFT JOIN hg3_cat ON hg3_img.id_cat=hg3_cat.id  WHERE hg3_img.id='.intval($v).' ORDER BY hg3_cat.name', TRUE);
...
$i=0;
$tn_arr=array();
$req=$sql->query('SELECT hg3_img.id AS id_img, hg3_img.file, hg3_img.name AS name_img, hg3_cat.id AS id_cat, hg3_cat.name AS name_cat, hg3_cat.link
